        ...
        for product in products:
            with st.container():
                ...
                if price_history:
                    ...
                    col3.metric("Current Price", f"{latest_price:.2f} {self.product_service.repository.get(product.url).currency}", delta=f"{latest_price - price_history[-2].price:.2f} {self.product_service.repository.get(product.url).currency}" if len(price_history) > 1 else "N/A")
                else:
